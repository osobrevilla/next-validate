  ...
  if (type === "string") {
    if (isNaN(param)) {
      param
        ...
        .forEach(row => {
          let value = row[1];
          args[row[0].trim()] = isNaN(value)
            ? isBoolean.test((value = value.trim().toLowerCase()))
              ? value === "true"
              : value
            : Number(value);
        });
...
const _getFormFields = form =>
  [].slice.call(form.querySelectorAll(_getDataSelectors().join(", ")));

const _getDataSelectors = () =>
  Object.keys(Validators.getRules()).map(frag => `[data-validate-${frag}]`);

const _getFieldValue = el =>
...
const _getMessage = (fieldObject, field) => {
  let _message = Object.assign({}, fieldObject.runtimeOptions, fieldObject.args)
    .message;
  // console.log("row", fieldObject.runtimeOptions, fieldObject.args);
  return typeof _message === "function"
    ? _message.call(fieldObject.runtimeOptions, fieldObject.args, field)
    : _message;
};
...
class NextValidate {
        ...
        for (let name in directives.attrs) {
          if (Validators[name]) {
            if (directives.attrs[name]) {
              validatesFns.push(
                Validators[name.toLowerCase()](
                  _getArgsFromDirective(directives.attrs[name])
                )
              );
            } else {
  ...
  getFormGroup(field) {
    var formGroup = utils.closest(
      field,
      ".form-group, .nextvalidate-form-group"
    );
  ...
  _bindEvents() {
    ["focusin", "focusout", "keyup", "click"].forEach(eventName =>
      this.form.addEventListener(eventName, this._validateEvtHandler, false)
    );
  }

  validateElement(element) {
    return this.isList(element.field)
      ? this.validateFieldList(element)
  ...
  _buildMessageBlock(message) {
    ...
    return `<div class="form-control-feedback">${this.options
      .errorMessageTemplate || GLOBAL_MESSAGE_ERROR_TEMPLATE}</div>`.replace(
      "{{MESSAGE}}",
      message
    );
  }
  ...
  _putMessage(formGroup, message) {
    ...
    if (message) {
      ...
      formGroup.appendChild(
        utils.elementFromString(this._buildMessageBlock(message))
      );
    }
  }
  ...
  displayMessage(field, message, args, fadeOutTime) {
    this._putMessage(
      this.getFormGroup(field),
      this._buildMessage(message, args)
    );
    if (fadeOutTime) {
  ...
  validateFieldList(fieldObject) {
    ...
    let testObjects = fieldObject.testObjects.filter(
      t => ["required", "between"].indexOf(t.name) !== -1
    );
    var testObj;
    switch (type) {
      case "radio":
  ...
  validateField(fieldObject) {
    let field = Array.isArray(fieldObject.field)
      ? fieldObject.field[0]
      : fieldObject.field;
    // if input is invisible, skip and return true.
    if (field.offsetParent === null || field.hasAttribute("disabled"))
      return true;
  ...
  isValid() {
    ...
    [].forEach.call(
      this._validators_,
      validate => (isValid = isValid * this.validateElement(validate))
    );
    return isValid;
  }
...
Validators.add("between", {
  test: function(value, el, args) {
    if (NextValidate.prototype.isList(el)) {
      ...
      return (
        parseInt(value) >= parseInt(args.min) &&
        parseInt(value) <= parseInt(args.max)
      );
    }
    return true;
  },
  ...
  .add("require", REQUIRED_RULE)
  .add("required", REQUIRED_RULE)
  .add("alpha", {
    pattern: /^[a-zA-Z\s']+$/,
    test(value, el) {
      return this.pattern.test(value);
  ...
  .add("minlength", {
    ...
    message: function(args, field) {
      return `mínimo ${args.value} caracteres <span style="float:right">${
